...
(define val-decl?
  (lambda (decl)
    (cases declaration decl (val-decl (name ty) #t) (else #f))))
...
(define opaque-type-decl?
  (lambda (decl)
    (cases declaration decl (opaque-type-decl (name) #t) (else #f))))
  ...
  (lambda (m-name var-name tenv)
    (let ([iface (lookup-module-name-in-tenv tenv m-name)])
      (cases
       ...
       (proc-iface
        (param-name param-iface result-iface)
        (eopl:error
    ...
    (let recur ([tenv tenv])
      (cases type-environment
             ...
             (extend-tenv-with-type
              (name type saved-tenv)
              (if (eqv? name search-sym) type (recur saved-tenv)))
  ...
  (lambda (m-name t-name tenv)
    (let ([iface (lookup-module-name-in-tenv tenv m-name)])
      (cases
       ...
       (proc-iface
        (bvar bvar-iface result-iface)
        (eopl:error
         'lookup-qualified-type
...
(define expand-type
  (lambda (ty tenv)
    (cases type
           ...
           (named-type (name) (lookup-type-name-in-tenv tenv name))
           (qualified-type
            (m-name t-name)
        ...
        (cases
         ...
         (opaque-type-decl
          ...
          (let ([expanded-type (qualified-type m-name t-name)])
            (let ([new-env (extend-tenv-with-type t-name
                                                  expanded-type
                                                  internal-tenv)])
...
(define raise-bad-module-application-error!
  (lambda (expected-type rand-type body)
    (eopl:pretty-print (list 'bad-module-application
                             body
                             'actual-rand-interface:
                             rand-type
                             'expected-rand-interface:
                             expected-type))
    (eopl:error 'interface-of "Bad module application ~s" body)))
        ...
        (cases
         ...
         (type-defn
          ...
          (let ([ty (expand-type ty tenv)])
            (let ([new-env (extend-tenv-with-type name ty tenv)])
...
(define rename-in-iface
  (lambda (m-type old new)
    (cases
     ...
     m-type
     (simple-iface (decls)
                   (simple-iface (rename-in-decls decls old new)))
     (proc-iface
      (param-name param-type result-type)
        ...
        (let ([decl (car decls)] [decls (cdr decls)])
          (cases
           ...
           (transparent-type-decl
            (name ty)
            (cons
             (transparent-type-decl name (rename-in-type ty old new))
             (if (eqv? name old)
                 decls
                 (rename-in-decls decls old new)))))))))
...
(define rename-in-type
  (lambda (ty old new)
    (let recur ([ty ty])
      (cases type
             ty
             (named-type (id) (named-type (rename-name id old new)))
             (qualified-type
              (m-name name)
              (qualified-type (rename-name m-name old new) name))
             (proc-type (t1 t2) (proc-type (recur t1) (recur t2)))
             (else ty) ; this covers int, bool, and unknown.
             ))))

(define rename-name
  (lambda (name old new) (if (eqv? name old) new name)))
...
(define value-of-module-body
  (lambda (m-body env)
    (cases
     ...
     (var-module-body (id) (lookup-module-name-in-env id env))
...
(check-equal? (:t str14) 'int)
(check-equal? (:e str14) (num-val 44))
...
(check-equal? (:t str15) 'int)
(check-equal? (:e str15) (num-val 0))