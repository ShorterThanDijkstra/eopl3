...
(define the-grammar
  '((program ((arbno module-definition)
    ...
    (module-body ((arbno module-definition)
                  "["
                  (arbno definition)
                  "]")
                 defns-module-body)
    ...
    (expression ("letrec" type
                ...
                letrec-exp)
    (expression
     ("from" expression "take" identifier)
     qualified-var-exp)
...
(define lookup-qualified-var-in-module-type
  (lambda (var m-type)
    (cases
     type
     m-type
     (module-type
      ...
      (let loop ([vars vars] [types types])
        (if (null? vars)
            (eopl:error
             'lookup-qualified-var-in-module-type)
            (if (eqv? (car vars) var)
                (car types)
...
(define expval->typed-module
  (lambda (v)
    (cases expval
           v
           (module-val (mod) mod)
           (else (expval-extractor-error 'module
                                         v)))))
...
(define lookup-qualified-var-in-module-val
  (lambda (var-name m-val)
    (cases typed-module
           (expval->typed-module m-val)
           (simple-module
            (bindings)
...
(define type-of-program
  (lambda (pgm)
    (cases program
           ...
           (a-program
            (module-defs body)
            (let ([tenv (add-module-defns-to-tenv
                         module-defs
                         (init-tenv))])
...
(define interface-of
  (lambda (m-body tenv)
    (cases
     ...
     (defns-module-body
      (module-defs defns)
      (let ([new-tenv (add-module-defns-to-tenv
                       module-defs
                       tenv)])
        (simple-iface
         (defns-to-decls defns new-tenv)))))))
         ...
         (a-module-definition
          ...
          (let ([env (extend-env-with-module
                      ...
                      (value-of-module-body m-body
                                            env)
                      env)])
            (add-module-defns-to-env (cdr defs)
                                     env)))))))
...
(define value-of-module-body
  (lambda (m-body env)
    (cases
     ...
     (defns-module-body
      (module-defs defns)
      (let ([new-env (add-module-defns-to-env
                      module-defs
                      env)])
        (simple-module
...
(define value-of
  (lambda (exp env)
    ...
    (cases
     ...
     (qualified-var-exp
      (m-exp var-name)
      (let ([m-val (value-of m-exp env)])
        (lookup-qualified-var-in-module-val
         var-name
         m-val)))
     (diff-exp (exp1 exp2)
               (let ([val1 (expval->num
...
(define str6
  "module m1
     ...
     [u = 44
      n = m2]
 from from m1 take n take v")
(check-equal? (:e str6) (num-val 33))
  ...
  (simple-module
   (extend-env
    ...
    (extend-env
     'u
     (bool-val #f)
     (extend-env 'v (num-val -4) (empty-env)))))))
(check-equal? (:t str7) '((u : bool) (v : int)))
