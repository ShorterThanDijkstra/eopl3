        ...
        (cases module-definition
          ...
          (a-module-definition (m-name iface m-body)
                               (add-module-defns-to-env
                                ...
                                (extend-env-with-module
                                 m-name
                                 (value-of-module-body m-body env iface)
                                 env)))))))

;; value-of-module-body : ModuleBody * Env -> TypedModule
(define value-of-module-body
  (lambda (m-body env iface)
    (cases module-body
      m-body
      (defns-module-body (defns)
        (simple-module (defns-to-env defns env iface))))))
          ...
          (val-defn
           ...
           (let ([val (value-of exp env)])
             ...
             (let ([new-env (extend-env var val env)])
               (if (in-interface? var iface)
                   (extend-env var val (defns-to-env (cdr defns) new-env iface))
                   (defns-to-env (cdr defns) new-env iface))
               )))))))
