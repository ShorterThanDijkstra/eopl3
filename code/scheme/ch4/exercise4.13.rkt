  ...
  (lambda (proc1 vals store)
    (cases proc proc1
      (procedure (vars body saved-env)
                 (value-of body (extend-env-list vars vals saved-env) store)))))
    ...
    (cases expression exp
      ...
      (call-exp (rator rands)
                (let* ((ans1 (value-of rator env store))
                       ...
                       (store1 (answer->store ans1)))
                  (letrec ((loop-rands (lambda (rands args storeN)
                                      (if (null? rands)
                                          (list (reverse args) storeN)
                                          (let* ((ans (value-of (car rands) env storeN))
                                                 (arg (answer->expval ans))
                                                 (new-store (answer->store ans)))
                                            (loop-rands (cdr rands) (cons arg args) new-store))))))
                    (let* ((args&store (loop-rands rands '() store1))
                           (args (car args&store))
                           (new-store (cadr args&store)))
...
(define str3
  "let delta = newref(114)
   in let sumDelta = proc (u v w)
              let uvw = -(u, -(0, -(v, -(0,w))))
              in -(uvw,-(0, deref(delta)))
      in (sumDelta 5 1 4)")
(check-equal? (run str3) (num-val 124))
