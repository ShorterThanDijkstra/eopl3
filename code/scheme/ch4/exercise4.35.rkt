...
(define-datatype
  ...
  (setref-exp (var1 identifier?) (expr expression?)))
...
(define value-of
  (lambda (exp env)
    (cases
      ...
      (deref-exp (var)
        (let ((ref (expval->ref (deref (apply-env env var)))))
          (deref ref)))
      (setref-exp (var expr)
                  (let ((val (value-of expr env))
                        (ref (expval->ref (deref (apply-env env var)))))
                    (setref! ref val)))
      )))
...
(check-equal? (run str6) (num-val 1))
