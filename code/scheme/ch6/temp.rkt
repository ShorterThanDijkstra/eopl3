...
;; Exercise 6.40 [★★] Implement try/catch and throw from section 5.4 by adding them to the CPS translator. You should
;; not need to add anything to CPS-OUT. Instead, modify cps-of-exp to take two continuations: a success continuation and
...
(define cps-of-if-exp
  (lambda (exp1 exp2 exp3 k-exp h-exp)
    (cps-of-exps (list exp1)
                 h-exp
                 (lambda (new-rands)
                   (cps-if-exp (car new-rands) (cps-of-exp exp2 k-exp h-exp) (cps-of-exp exp3 k-exp h-exp))))))

(define cps-of-let-exp
  (lambda (id rhs body k-exp h-exp)
    (cps-of-exps (list rhs)
                 h-exp
                 (lambda (new-rands)
                   (cps-let-exp id (car new-rands) (cps-of-exp body k-exp h-exp))))))

(define cps-of-letrec-exp
  (lambda (proc-names idss proc-bodies body k-exp h-exp)
    (cps-letrec-exp proc-names
                    (map (lambda (ids)
                           (append ids (list 'k%00 'h%00)))
                         idss)
                    (map (lambda (exp)
                           (cps-of-exp exp (cps-var-exp 'k%00) (cps-var-exp 'h%00)))
                         proc-bodies)
                    (cps-of-exp body k-exp h-exp))))

(define cps-of-call-exp
  (lambda (rator rands k-exp h-exp)
    (cps-of-exps (cons rator rands)
                 h-exp
                 (lambda (new-rands)
                   (cps-call-exp (car new-rands) (append (cdr new-rands) (list k-exp h-exp)))))))
      ...
      [proc-exp (vars body) (make-send-to-cont k-exp
                                               (cps-proc-exp (append vars (list 'k%00 'h%00))
                                                             (cps-of-exp body (cps-var-exp 'k%00)
                                                                         (cps-var-exp 'h%00))))]
      [zero?-exp (exp1) (cps-of-zero?-exp exp1 k-exp h-exp)]
      [diff-exp (exp1 exp2) (cps-of-diff-exp exp1 exp2 k-exp h-exp)]
      [sum-exp (exps) (cps-of-sum-exp exps k-exp h-exp)]
      [if-exp (exp1 exp2 exp3) (cps-of-if-exp exp1 exp2 exp3 k-exp h-exp)]
      [let-exp (var exp1 body) (cps-of-let-exp var exp1 body k-exp h-exp)]
      [letrec-exp (ids bidss proc-bodies body) (cps-of-letrec-exp ids bidss proc-bodies body k-exp h-exp)]
      [call-exp (rator rands) (cps-of-call-exp rator rands k-exp h-exp)]
      [try-exp (exp1 var exp2) (cps-of-try-exp exp1 var exp2 k-exp h-exp)]
      [raise-exp (exp1) (cps-of-raise-exp exp1 k-exp h-exp)]
  ...
  (lambda (pgm)
    (cases program pgm
      [a-program (exp1) (cps-a-program (cps-of-exps (list exp1)
                                                    (cps-proc-exp (list 'v)
                                                                  (cps-printk-exp (cps-var-exp 'v)
                                                                                  (simple-exp->exp (cps-const-exp 1))))
                                                    (lambda (new-args)
