...
#|
list-index even? '(7 3 2 5 11 17) => 2
list-index-last even? '(7 3 2 5 11 17) => 3
|#
...
(define cps-of-exps
  (lambda (exps builder)
    (let cps-of-rest ([exps exps])
      ...
      (let ([pos (list-index-last (lambda (exp) (not (inp-exp-simple? exp))) exps)])
        (if (not pos)
            (builder (map cps-of-simple-exp exps))
            (let ([var (fresh-identifier 'var)]
                  [pos (- (- (length exps) 1) pos)])
