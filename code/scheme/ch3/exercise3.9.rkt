...
;             emptylist))
(define code-ast1
  (let-exp 'x (const-exp 4) (cons-exp (var-exp 'x)
                                      (cons-exp (cons-exp (diff-exp (var-exp 'x)
                                                                    (const-exp 1))
                                                          (emptylist-exp))
                                                (emptylist-exp)))))

(check-equal? (value-of code-ast1 (init-env)) (list-val '(4 (3))))

(define code-ast2 (car-exp code-ast1))
(check-equal? (value-of code-ast2 (init-env)) (num-val 4))

(define code-ast3 (cdr-exp code-ast1))
(check-equal? (value-of code-ast3 (init-env)) (list-val '((3))))

(define code-ast4 (null?-exp code-ast1))
(check-equal? (value-of code-ast4 (init-env)) (bool-val #f))

(define code-ast5 (null?-exp (emptylist-exp)))
(check-equal? (value-of code-ast5 (init-env)) (bool-val #t))
