...
;;;Continuation
(define frame list)
(define end-cont? null?)
(define (frame-name frame) (car frame))
(define (frame-1 frame) (cadr frame))
(define (frame-2 frame) (caddr frame))
(define (frame-3 frame) (cadddr frame))

(define (end-cont) '())
(define (zero1-cont cont)
  (cons (frame 'zero1-cont) cont))
(define (let-exp-cont var body env cont)
  (cons (frame 'let-exp-cont var body env) cont))
(define (if-test-cont exp2 exp3 env cont)
  (cons (frame 'if-test-cont exp2 exp3 env) cont))
(define (diff1-cont exp2 env cont)
  (cons (frame 'diff1-cont exp2 env) cont))
(define (diff2-cont val1 cont)
  (cons (frame 'diff2-cont val1) cont))
(define (rator-cont rand env cont)
  (cons (frame 'rator-cont rand env) cont))
(define (rand-cont val1 cont)
  (cons (frame 'rand-cont val1) cont))

; apply-cont : Cont × ExpVal → FinalAnswer
(define apply-cont
  (lambda (cont val)
    (if (end-cont? cont)
        (begin (eopl:printf "End of computation:~s.~%" val)
               val)
        (let ((curr (car cont))
              (name (frame-name (car cont)))
              (saved-cont (cdr cont)))
          (cond [(eqv? name 'zero1-cont)
                 (apply-cont saved-cont
                             (bool-val
                              (zero? (expval->num val))))]
                [(eqv? name 'let-exp-cont)
                 (let ([var (frame-1 curr)]
                       [body (frame-2 curr)]
                       [saved-env (frame-3 curr)])
                   (value-of/k body
                               (extend-env var val saved-env) saved-cont))]
                [(eqv? name 'if-test-cont)
                 (let ([exp2 (frame-1 curr)]
                       [exp3 (frame-2 curr)]
                       [saved-env (frame-3 curr)])
                   (if (expval->bool val)
                ...
                [(eqv? name 'diff2-cont)
                 (let ([val1 (frame-1 curr)])
                   (let ((num1 (expval->num val1))
                         (num2 (expval->num val)))
                     (apply-cont saved-cont
                                 (num-val (- num1 num2)))))]
                [(eqv? name 'rator-cont)
                 (let ([rand (frame-1 curr)]
                       [env (frame-2 curr)])
                   (value-of/k rand env
