    ...
    (cases continuation cont
      ...
      (zero1-cont (saved-cont)
                  (apply-cont saved-cont
                              (bool-val
                               (zero? (expval->num val)))))
      (let-exp-cont (vars exps vals body env saved-cont)
       (if (null? exps)
           (value-of/k body (extend-env-vars-vals vars (reverse (cons val vals)) env) saved-cont)
      ...
      (let3-exp1-cont
       ...
       (let ((new-env (extend-env var1 val env)))
         (value-of/k exp2 new-env (let3-exp2-cont var2 var3 exp3 body new-env cont))))
      (let3-exp2-cont (var2 var3 exp3 body env cont)
      ...
      (rands-cont (p rands vals env cont)
       (define new-vals (cons val vals))
       (if (null? rands)
           (apply-procedure/k (expval->proc p) (reverse new-vals) cont)
           (value-of/k (car rands) env (rands-cont p (cdr rands) new-vals env cont))))
      (cons-fst-cont (snd-exp env cont)
                     (value-of/k snd-exp env (cons-snd-cont val cont)))
      (cons-snd-cont (val1 cont)
      ...
      (cdr-cont (cont)
                (let ((fst (expval->pair-snd val)))
                  (apply-cont cont fst)))
      (null?-cont (cont)
                  (apply-cont cont (bool-val (nil? val))))

      (list-cont (exps vals env cont)
                 (if (null? exps)
                     (apply-cont cont (list->pair-vals (reverse (cons val vals))))
                     (value-of/k (car exps) env (list-cont (cdr exps) (cons val vals) env cont))))
      )))
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      ...
      (car-exp (pair-exp)
               (value-of/k pair-exp env (car-cont cont)))
      (cdr-exp (pair-exp)
               (value-of/k pair-exp env (cdr-cont cont)))
      (null?-exp (expr)
                 (value-of/k expr env (null?-cont cont)))
