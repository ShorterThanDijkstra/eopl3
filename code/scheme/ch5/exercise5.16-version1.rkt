...
(define apply-command-cont (lambda (cont val) (cont val)))

(define end-cont (lambda () (lambda (_) (eopl:printf "End of computation.~%"))))

(define assign-stmt-cont
  (lambda (var env saved-cont)
    (lambda (val)
      (begin
        (setref! (apply-env env var) val)
        (apply-command-cont saved-cont val)))))

(define print-stmt-cont
  (lambda (saved-cont)
    (lambda (val)
      (begin
...
(define seq-stmt-cont
  (lambda (stmts env saved-cont)
    (lambda (_)
      (if (null? stmts)
          (apply-command-cont saved-cont (nil-val))
...
(define if-stmt-cont
  (lambda (conseq alter env saved-cont)
    (lambda (val)
      (if (expval->bool val)
          (result-of/k conseq env saved-cont)
          (result-of/k alter env saved-cont)))))
(define while-pred-cont
  (lambda (pred stmt env saved-cont)
    (lambda (val)
      (if (expval->bool val)
          (result-of/k stmt env (while-stmt-cont pred stmt env saved-cont))
          (apply-command-cont saved-cont (nil-val))))))

(define while-stmt-cont
  (lambda (pred stmt env saved-cont)
    (lambda (val)
      (value-of/k pred env (while-pred-cont pred stmt env saved-cont)))))

(define cont? (lambda (cont) (or (procedure? cont) (expr-cont? cont))))

(define apply-cont
  (lambda (cont val)
    (if (expr-cont? cont)
        (apply-expr-cont cont val)
        (apply-command-cont cont val))))
