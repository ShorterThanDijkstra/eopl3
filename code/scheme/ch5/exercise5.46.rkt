...
(define enqueue (lambda (queue th) (append queue (list (cons th the-time-remaining)))))

(define dequeue (lambda (queue f)
                  (let ((fst (car queue))
                        (rest (cdr queue)))
                    (let ((remaining (if (zero? (cdr fst)) the-max-time-slice (cdr fst)))
                          (th (car fst)))
                      (begin
                        (newline)
                        (display "===================")
                        ...
                        (set! the-time-remaining remaining)
                        (f th rest))))))
