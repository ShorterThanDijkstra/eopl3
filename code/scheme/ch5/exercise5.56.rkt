           ...
           (if (empty? wait-queue)
               ...
               (dequeue wait-queue
                        (lambda (first-waiting-th other-waiting-ths)
                          (place-on-ready-queue! first-waiting-th the-store)
...
(define resume-thread
  (lambda (th)
    (cases thread th
      (cont-thread (id saved-cont val saved-store)
                   (begin (set! the-current-thread-id id)
                          (set! the-store saved-store)
                          (apply-cont saved-cont val)))
      (procedure-thread (id proc1 val saved-cont saved-store)
                        (begin (set! the-current-thread-id id)
                               (set! the-store saved-store)
  ...
  (lambda (thread-id msg cont)
    (let ([threads (filter (lambda (th) (= (get-thread-id th) thread-id)) the-ready-queue)])
      (if (null? threads)
          ...
          (begin
            (place-on-ready-queue! (cont-thread the-current-thread-id cont (num-val 73) the-store))
...
(define get-thread-id
  (lambda (th)
    (cases thread th
      (cont-thread (id saved-cont val saved-store) id)
      (procedure-thread (id proc1 val saved-cont saved-store) id))))
...
(define apply-cont
  (lambda (cont val)
    ...
    (if (time-expired?)
        (begin
          (place-on-ready-queue! (cont-thread the-current-thread-id cont val the-store))
          (run-next-thread))
        (begin
          (decrement-timer!)
          (cases
            ...
            (spawn-cont
             ...
             (let ([proc1 (expval->proc val)]
               ...
               (place-on-ready-queue!
                (procedure-thread id proc1 (num-val id) (end-subthread-cont) the-store))
            ...
            (wait-cont (saved-cont)
                       (wait-for-mutex
                        (expval->mutex val)
                        (cont-thread the-current-thread-id saved-cont (num-val 52) the-store)))
            (signal-cont (saved-cont)
                         (signal-mutex
                          (expval->mutex val)
                          (cont-thread the-current-thread-id saved-cont (num-val 53) the-store)))
...
(define str13
  "let mut = mutex()
   ...
   in begin send(r3, 11); send(r3, 13) end
    ")
