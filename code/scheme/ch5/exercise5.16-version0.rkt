...
(define-datatype
 ...
 (if-test-cont (exp2 expression?)
               ...
               (env environment?)
               (saved-cont cont?))
 (diff1-cont (exp2 expression?) (env environment?) (saved-cont cont?))
 (diff2-cont (val1 expval?) (saved-cont cont?))
 (add1-cont (exp2 expression?) (env environment?) (saved-cont cont?))
 (add2-cont (val1 expval?) (saved-cont cont?))
 (mul1-cont (exp2 expression?) (env environment?) (cont cont?))
 (mul2-cont (val1 expval?) (saved-cont cont?))
 (rator-cont (rands (list-of expression?)) (env environment?) (saved-cont cont?))
 (rands-cont (p expval?)
             (rands (list-of expression?))
             (vals (list-of expval?))
             (env environment?)
             (saved-cont cont?))
 (cons-fst-cont (snd-exp expression?) (env environment?) (saved-cont cont?))
 (cons-snd-cont (val1 expval?) (saved-cont cont?))
...
(define apply-expr-cont
  (lambda (cont val)
    (cases
     ...
     (add1-cont (exp2 env saved-cont) (value-of/k exp2 env (add2-cont val saved-cont)))
     (add2-cont (val1 saved-cont)
                (let ([num1 (expval->num val1)] [num2 (expval->num val)])
                  (apply-cont saved-cont (num-val (+ num1 num2)))))
     (mul1-cont (exp2 env saved-cont) (value-of/k exp2 env (mul2-cont val saved-cont)))
     ...
     (cons-snd-cont (val1 saved-cont) (apply-cont saved-cont (pair-val val1 val)))
     (car-cont (saved-cont)
