...
(define refto-type?
  (lambda (ty) (cases type ty [refto-type (ty) #t] [else #f])))
...
(define apply-one-subst
  (lambda (ty0 tvar ty1)
    (cases type
           ...
           (bool-type () (bool-type))
           (void-type () (void-type))
           (refto-type (ty) (refto-type (apply-one-subst ty tvar ty1)))
           (proc-type (arg-type result-type)
...
(define apply-subst-to-type
  (lambda (ty subst)
    (cases
     ...
     (bool-type () (bool-type))
     (void-type () (void-type))
     (refto-type (ty) (refto-type (apply-subst-to-type ty subst)))
     (proc-type (t1 t2)
    ...
    (let ([ty1 (apply-subst-to-type ty1 subst)]
      ...
      (cond
        ...
        [(and (refto-type? ty1) (refto-type? ty2))
         (unifier (refto-type->type ty1) (refto-type->type ty2) subst exp)]
...
(define no-occurrence?
  (lambda (tvar ty)
    (cases type
           ...
           (bool-type () #t)
           (void-type () #t)
           (refto-type (ty) (no-occurrence? tvar ty))
           (proc-type (arg-type result-type)
     ...
     (newref-exp (exp1)
                 (cases answer
                        ...
                        (an-answer (exp1-ty subst)
                                   (an-answer (refto-type exp1-ty) subst))))
     (deref-exp
      (ref-exp)
      (cases answer
             (type-of ref-exp tenv subst)
             (an-answer
              (ref-exp-ty subst)
              (let ([tvar1 (fresh-tvar-type)])
                (let ([subst (unifier (refto-type tvar1) ref-exp-ty subst exp)])
     ...
     (setref-exp
      ...
      (cases answer
             ...
             (an-answer
              (ref-exp-ty subst)
              (cases answer
                     (type-of val-exp tenv subst)
                     (an-answer (val-exp-ty subst)
                                (let ([subst (unifier ref-exp-ty
                                                      (refto-type val-exp-ty)
                                                      subst
                                                      exp)])
...
(define str12
  "proc(x: ?)
     proc (z: ?) let zz = newref(-(deref(z), deref(x)))
                 in setref(z, deref(zz))
   ")
(type-eq? str12 '((refto int) -> ((refto int) -> void)))
