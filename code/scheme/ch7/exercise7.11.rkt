...
(define type-of
  (lambda (exp tenv)
    (cases
      ...
      (newpair-exp (exp1 exp2) (pair-type (type-of exp1 tenv) (type-of exp2 tenv)))
      (left-exp (exp1) (let ([ty (type-of exp1 tenv)])
                         (cases type ty
                           (pair-type (ty1 ty2)
                                      ty1)
                           (else (eopl:error 'type-of "expected a pair type, got: ~a" ty)))))
      (right-exp (exp1) (let ([ty (type-of exp1 tenv)])
                          (cases type ty
                            (pair-type (ty1 ty2)
                                       ty2)
                            (else (eopl:error 'type-of "expected a pair type, got: ~a" ty)))))
      (setleft-exp (exp1 exp2) (let ([pair-ty (type-of exp1 tenv)]
                                     [val-ty (type-of exp2 tenv)])
                                 (cases type pair-ty
                                   (pair-type (ty1 _)
                                              (check-equal-type! ty1 val-ty exp2)
                                              (void-type))
                                   (else (eopl:error 'type-of "expected a pair type, got: ~a" pair-ty)))))

      (setright-exp (exp1 exp2) (let ([pair-ty (type-of exp1 tenv)]
                                      [val-ty (type-of exp2 tenv)])
                                  (cases type pair-ty
                                    (pair-type (_ ty2)
                                               (check-equal-type! ty2 val-ty exp2)
                                               (void-type))
                                    (else (eopl:error 'type-of "expected a pair type, got: ~a" pair-ty)))))
    (call-exp
     (rator rand)
