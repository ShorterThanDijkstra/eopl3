...
(define nil-val? (lambda (v) (cases expval v (nil-val () #t) (else #f))))
...
(define type-of
  (lambda (exp tenv)
    (cases
      ...
      (list-exp (fst rest)
                (let ([ty-fst (type-of fst tenv)])
                  (for-each (lambda (exp)
                              (check-equal-type! ty-fst (type-of exp tenv) exp))
                            rest)
                  (list-type ty-fst)))
      (cons-exp (fst rest)
                (let ([ty-fst (type-of fst tenv)]
                      [ty-rest (type-of rest tenv)])
                  (cases type ty-rest
                    (list-type (ty)
                               (check-equal-type! ty-fst ty rest)
                               ty-rest)
                    (else (eopl:error 'type-of "expected a list type, got: ~s" ty-rest)))))

      (null?-exp (exp1)
                 (let ([exp1-type (type-of exp1 tenv)])
                   (cases type exp1-type
                     (list-type (_) (bool-type))
                     (else (eopl:error 'type-of "expected a list type, got: ~s" exp1-type)))))
...
(check-equal? (:t "list(1, 2, 3)") '(list-type int))
; (:t "list(1, true") ; should fail

(check-equal? (:t "cons(zero?(1), list(false, true))") '(list-type bool))
; (:t "cons(true, list(1, 2, 3))")  ; should fail

(check-equal? (:t "cons(1, emptylist: int)") '(list-type int))
; (:t "cons(1, emptylist: bool)")  ; should fail

(check-equal? (:t "null?(emptylist: int)") 'bool)
; (:t "null?(1)") ; should fail

(check-equal? (:t "car(list(1, 2))") 'int)
(check-equal? (:t "car(emptylist: bool)") 'bool)

(check-equal? (:t "cdr(list(1, 2))") '(list-type int))
(check-equal? (:t "cdr(emptylist: bool)") '(list-type bool))

;  Why would it be unreasonable for the rules to guarantee that the lists be non-empty?
;  Because it is hard to check whether the result of cdr(<<list-exp>>) is empty or non-empty.

;  Why is the type parameter in emptylist necessary?
;  Because we need know the type of expressions like cdr(cons(1, emptylist))
