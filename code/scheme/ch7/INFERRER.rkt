...
(check-true (type-eq? str0 '(t37 -> t37)))
(check-false (type-eq? str0 '(t73 -> t37)))

(define str1 "proc (x: ?) -(x, 3)")
(check-true (type-eq? str1 '(int -> int)))

(define str2 "proc (x: ?) proc (y: ?) (x y)")
(check-true (type-eq? str2 '((t0 -> t1) -> (t0 -> t1))))

(define str3 "proc (x: ?) (x 3)")
(check-true (type-eq? str3 '((int -> t0) -> t0)))

(define str4 "proc (x: ?) (x x)")
; (:t str4) ; should fail

(define str5 "proc (x: ?) if x then 88 else 99")
(check-true (type-eq? str5 '(bool -> int)))

(define str6 "proc (x: ?) proc (y: ?) if x then y else 99")
(check-true (type-eq? str6 '(bool -> (int -> int))))
...
(check-true (type-eq? str8 '((t1 -> int) -> ((int -> int) -> ((int -> bool) -> (t1 -> int))))))

(define str9 " proc (x: ?)
                 proc(p: ?)
                   proc (f: ?)
                     if (p x) then -(x,1) else (f p)")
(check-true (type-eq? str9 '(int -> ((int -> bool) -> (((int -> bool) -> int) -> int)))))
...
(define str11
  "letrec
   ...
   in double")
(check-true (type-eq? str11 '(int -> int)))
