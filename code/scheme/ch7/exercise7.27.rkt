...
(define gen-subst
  (lambda (exp tenv subst)
    (cases
        expression
      exp
      (const-exp (num) (let ([tvar1 (fresh-tvar-type)])
                         (extend-subst subst tvar1 (int-type))))
      (zero?-exp
       (exp1) (let ([subst (gen-subst exp1 tenv subst)])
                (let ([exp1-tvar (car (subst-fst subst))])
                  (let ([subst (extend-subst subst
                                             exp1-tvar
                                             (int-type))]
                        [tvar1 (fresh-tvar-type)])
                    (extend-subst subst tvar1 (bool-type))))))
      (diff-exp
       (exp1 exp2) (let* ([subst (gen-subst exp1 tenv subst)]
                          [exp1-tvar (car (subst-fst subst))])
                     (let* ([subst (gen-subst exp2 tenv (extend-subst subst exp1-tvar (int-type)))]
      ...
      (if-exp
       (exp1 exp2 exp3) (let* ([subst (gen-subst exp1 tenv subst)]
                               [exp1-tvar (car (subst-fst subst))])
                          (let* ([subst (gen-subst exp2
                                                   tenv
                                                   (extend-subst subst
                                                                 exp1-tvar
                                                                 (bool-type)))]
      ...
      (let-exp (var exp1 body) (let* ([subst (gen-subst exp1 tenv subst)]
                                      [exp1-tvar (car (subst-fst subst))])
                                 (gen-subst body (extend-tenv var exp1-tvar tenv)
                                            subst)))
      (proc-exp (var otype body)
                (let ([arg-type (otype->type otype)])
                  (let ([subst (gen-subst body (extend-tenv var arg-type tenv) subst)])
                    (let ([body-tvar (car (subst-fst subst))]
                          [tvar (fresh-tvar-type)])
                      (extend-subst  subst
                                     tvar
                                     (proc-type arg-type body-tvar))))))
                  ...
                  (let ([rator-tvar (car (subst-fst subst))])
                    (let ([subst (gen-subst rand tenv subst)])
                      (let ([rand-tvar (car (subst-fst subst))])
                        (let ([tvar (fresh-tvar-type)])
                          (extend-subst (extend-subst subst
                                                      rator-tvar
                                                      (proc-type rand-tvar tvar))
                                        tvar
                                        tvar)))))))
      ...
      (letrec-exp
       ...
       (let ([proc-arg-type (otype->type proc-arg-otype)]
         ...
         (let ([tenv-for-letrec-body (extend-tenv proc-name
                                                  ...
                                                  tenv)])
           (let ([subst (gen-subst proc-body
                                   (extend-tenv bvar proc-arg-type tenv-for-letrec-body)
                                   subst)])
             (let ([proc-body-tvar (car (subst-fst subst))])
               (let ([subst (extend-subst subst proc-body-tvar proc-result-type)])
