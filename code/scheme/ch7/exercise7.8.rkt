...
(define type-to-external-form
  (lambda (ty)
    (cases type
      ...
      (int-type () 'int)
      (bool-type () 'bool)
      (pair-type (ty1 ty2) (list 'pair-type
                                 (type-to-external-form ty1)
                                 '*
                                 (type-to-external-form ty2)))
...
(define type-of
  (lambda (exp tenv)
    (cases
      ...
      (newpair-exp (exp1 exp2) (let ([exp1-type (type-of exp1 tenv)]
      ...
      (unpair-exp (var1 var2 exp1 body)
                  (let ([exp1-type (type-of exp1 tenv)])
                    (cases type
                           ...
                           (else (eopl:error 'type-of "expected a pair type, got: ~s" (type-to-external-form exp1-type))))))
...
(define str9
  "let x = 11
   in newpair(x, zero?(x))
  ")
(check-equal? (:t str9) '(pair-type int * bool))
