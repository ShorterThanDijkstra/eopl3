...
(define initialize-class-decl!
  (lambda (c-decl)
    (cases class-decl
           c-decl
           (a-class-decl
            (c-name s-name f-names-host m-decls)
  ...
  (lambda (m self args)
    (cases method
           ...
           (a-method (vars body super-name field-names)
                    ...
                    ;  (eopl:pretty-print (last (length field-names) (map deref (object->fields self))))
                    ;  (display "----------------------\n")
                     (value-of body
                               (extend-env
                                vars
                                ...
                                (extend-env-with-self-and-super
                                 ...
                                 (extend-env field-names
                                            ;  (object->fields self)
                                             (last (length field-names) (object->fields self))
                                             (empty-env)))))))))
...
(define str9
  "class c1 extends object
   ...
   let o2 = new c2()
   in list(send o2 get_y())
   %error in list(send o2 get_x(), send o2 get_y())")
(check-equal? (:e str9) (list-val (list (num-val 5))))