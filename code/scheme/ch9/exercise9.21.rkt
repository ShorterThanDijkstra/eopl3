#lang eopl
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; hash map (dumb implementation)

(define (hash-symbol key size)
  (let ([initial 17] [mult 13])
    (let loop ([res initial]
               [cs (string->list (symbol->string key))])
      (if (null? cs)
          res
          (loop (modulo (+ res (* mult (char->integer (car cs))))
                        size)
                (cdr cs))))))
...
(define query-method
  (lambda (m-name hash-table)
    (let ([index (hash-symbol m-name (vector-length hash-table))])
      (let ([lst (vector-ref hash-table index)])
        (if lst
            (let ([name&method (assoc m-name lst)])
              (if name&method
                  (cadr name&method)
                  #f))
            #f)))))
...
(define put-method!
  (lambda (m-name method hash-table)
    (let ([index (hash-symbol m-name (vector-length hash-table))])
      (let ([lst (vector-ref hash-table index)])
        (if lst
            (let ([names (map car lst)])
              (let ([pos (location m-name names)])
                (if pos
                    (vector-set! hash-table index (list-set lst pos (list m-name method)))
                    (vector-set! hash-table index (cons (list m-name method) lst)))))
            (vector-set! hash-table index (list (list m-name method))))))))
...
(define method-environment? vector?)
...
(define find-method
  (lambda (c-name name)
    (let ([m-env (class->method-env (lookup-class c-name))])
      (let ([maybe-method (query-method name m-env)])
        (if maybe-method
            maybe-method
...
(define merge-method-envs!
  (lambda (super-m-env new-names&methods)
    (define put-methods!
      (lambda (names&methods hash-table)
        (for-each (lambda (name&method)
                    (put-method! (car name&method) (cadr name&method) hash-table))
                  names&methods)))

    (let ([new-size (+ (length new-names&methods) (vector-length super-m-env))])
      (let ([res (make-hash new-size)])
        (let ([copy-end (vector-length super-m-env)])
          (let loop ([index 0])
            (if (= index copy-end)
                (begin
                  (put-methods! new-names&methods res)
                  res)
                (let ([maybe-names&methods (vector-ref super-m-env index)])
                  (if maybe-names&methods
                      (begin (put-methods! maybe-names&methods res)
                             (loop (+ index 1)))
                      (loop (+ index 1))
                      )))))))))

; method-decls->method-env :
; Listof(MethodDecl) * ClassName * Listof(FieldName) -> Listof((m-name, method))
(define method-decls->method-list
...
(define initialize-class-env!
  (lambda (c-decls)
    (set! the-class-env (list (list 'object (a-class #f '() (make-hash 0)))))
         ...
         (add-to-class-env!
          ...
          (a-class s-name
                   ...
                   (merge-method-envs!
                    ...
                    (method-decls->method-list m-decls
                                               s-name
                                               f-names)))))))))
...
(define str6
  ...
  class c2 extends c1
    ...
    method m1 (u,v) -(-(x,u), -(y,v)) %-(-(31,7), -(12, 8))
    method m3 () 23
  class c3 extends c2
    field x
    field z
...
(define str10
  "class c1 extends object
   ...
   let o2 = new c2()
   in send o2 m2()")
(check-equal? (:e str10) (num-val 73))
#|
