...
(define type-of
  (lambda (exp tenv)
    (cases
     ...
     (fieldref-exp
      (exp1 f-name)
      (let ([c-name (type->class-name (type-of exp1 tenv))])
        (cases static-class
               (lookup-static-class c-name)
               (an-interface
                (method-tenv)
                (eopl:error 'type-of "expect a class, got ~s" c-name))
               (a-static-class
                (super-name i-names field-names field-types method-tenv)
                (let ([n (location f-name field-names)])
                  (if n
                      (list-ref field-types n)
                      (eopl:error 'type-of
                                  "no field ~s in class ~s"
                                  f-name
                                  c-name)))))))
     (fieldset-exp
      (exp1 f-name exp2)
      (let ([c-name (type->class-name (type-of exp1 tenv))])
        (cases static-class
               ...
               (a-static-class
                ...
                (let ([n (location f-name field-names)])
                  (if n
                      (let ([f-type (list-ref field-types n)]
                            [val-type (type-of exp2 tenv)])
                        (check-is-subtype! val-type f-type exp2)
                        (void-type))
                      (eopl:error 'type-of
                                  "no field ~s in class ~s"
                                  f-name
  ...
  (lambda (exp env)
    (cases
     ...
     (fieldset-exp (exp1 f-name exp2)
                   (let ([obj (value-of exp1 env)] [val (value-of exp2 env)])
                     (let ([f-names (class->field-names
                                     (lookup-class (object->class-name obj)))]
                       ...
                       (let ([index (location f-name f-names)])
                         (if index
                             (begin
                               (setref! (list-ref fields index) val)
                               obj)
                             (eopl:error 'value-of))))))
...
(check-equal? (:e str18) (list-val (list (num-val 3) (num-val 3))))
(check-equal? (:t str18) (list-type (int-type)))

(define str19
  "class c1 extends object
     field int x
     method void initialize() set x = 3
     method void setx(v: int) fieldset self x = v
   class c2 extends c1
     field int y
     method void initialize()
       begin
     ...
     method void sety(v: int) fieldset self y = v
     
    let o2 = new c2()
    in let ign = send o2 setx(37)
    in let ign = send o2 sety(73)
    in list(fieldref o2 x, fieldref o2 y)")
(check-equal? (:e str19) (list-val (list (num-val 37) (num-val 73))))
(check-equal? (:t str19) (list-type (int-type)))
...
(define str20
  "class c1 extends object
    ...
    let o1 = new c1()
    in let ign1 = fieldset o1 x = zero?(1)
    in fieldref o1 x")
; (:t str20) ;should fail
...
(define str21
 "class c1 extends object
   ...
   class c2 extends c1
    ...
    let o2 = new c2()
    in fieldref o2 x")
(check-equal? (:e str21) (bool-val #f))
(check-equal? (:t str21) (bool-type))
