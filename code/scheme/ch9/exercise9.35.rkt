  ...
  (lambda (pgm)
    (cases program
           ...
           (a-program (class-decls exp1)
                      ...
                      (check-no-initialize-call! exp1)
                      (type-of exp1 (init-tenv))))))

(define check-no-initialize-call!
  (lambda (exp)
    (cases
     expression
     exp
     ...
     (diff-exp (exp1 exp2)
               (check-no-initialize-call! exp1)
               (check-no-initialize-call! exp2)
               #t)
     (sum-exp (exp1 exp2)
              (check-no-initialize-call! exp1)
              (check-no-initialize-call! exp2)
              #t)
     (zero?-exp (exp1) (check-no-initialize-call! exp1) #t)
     (if-exp (test-exp true-exp false-exp)
             (check-no-initialize-call! test-exp)
             (check-no-initialize-call! true-exp)
             (check-no-initialize-call! false-exp)
             #t)
     (let-exp (ids rands body)
              (map check-no-initialize-call! rands)
              (check-no-initialize-call! body)
              #t)
     (proc-exp (bvars bvar-types body) (check-no-initialize-call! body) #t)
     (call-exp (rator rands)
               (check-no-initialize-call! rator)
               (map check-no-initialize-call! rands)
               #t)
     (letrec-exp
      (proc-result-types proc-names bvarss bvar-typess proc-bodies letrec-body)
      (map check-no-initialize-call! proc-bodies)
      (check-no-initialize-call! letrec-body)
      #t)
     (begin-exp (exp1 exps)
                (check-no-initialize-call! exp1)
                (map check-no-initialize-call! exps)
                #t)
     (assign-exp (id rhs) (check-no-initialize-call! rhs) #t)
     (null?-exp (exp1) (check-no-initialize-call! exp1) #t)
     (car-exp (exp1) (check-no-initialize-call! exp1) #t)
     (cdr-exp (exp1) (check-no-initialize-call! exp1) #t)
     (cons-exp (exp1 exp2)
               (check-no-initialize-call! exp1)
               (check-no-initialize-call! exp2)
               #t)
     ...
     (method-call-exp (obj-exp method-name rands)
                      ...
                      (if (eqv? method-name 'initialize)
                          (eopl:error 'check-no-initialize-call!
                                      "can not call initialize method")
                          #t))
     (super-call-exp (method-name rands)
                     ...
                     (if (eqv? method-name 'initialize)
                         (eopl:error 'check-no-initialize-call!
                                     "can not call initialize method")
                         #t))
     (cast-exp (exp1 class-name) (check-no-initialize-call! exp1) #t)
     (instanceof-exp (exp1 class-name) (check-no-initialize-call! exp1) #t))))
  ...
  (lambda (exp tenv)
    (cases
     ...
     (method-call-exp
      ...
      ; (if (eqv? method-name 'initialize)
      ;     (eopl:error 'type-of "can not call initialize method" )
      (let ([arg-types (types-of-exps rands tenv)]
...
(define str18
  "class c1 extends object
     field int x
     method void initialize() set x = 3
...
(define str19
  "class c1 extends object
     field int x
     method void initialize() set x = 3
     method void m1() send self initialize()
...
(define str20
  ...
  class c2 extends c1
     method void initalize() super initialize()
     method void m1() super initialize()
