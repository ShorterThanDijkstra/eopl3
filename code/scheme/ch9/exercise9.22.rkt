     ...
     (lambda (m-decl)
       (cases method-decl
         ...
         (a-method-decl
          ...
          (list (new-method-name method-name (length vars))
                (a-method vars (preprocess body) super-name field-names)))))
     m-decls)))
  ...
  (lambda (pgm)
    ...
    (cases program
      ...
      (a-program (class-decls body)
                 ...
                 (value-of (preprocess body) (init-env))))))
...
(define preprocess
  (lambda (exp)
    (cases expression
      ...
      (const-exp (num) exp)
      (var-exp (var) exp)
      (diff-exp (exp1 exp2) (diff-exp (preprocess exp1) (preprocess exp2)))
      (sum-exp (exp1 exp2) (sum-exp (preprocess exp1) (preprocess exp2)))
      (zero?-exp (exp1) (zero?-exp (preprocess exp1)))
      (if-exp (exp1 exp2 exp3) (if-exp (preprocess exp1) (preprocess exp2) (preprocess exp3)))
      (let-exp (vars exps body) (let-exp vars (map preprocess exps) (preprocess body)))
      (proc-exp (bvars body) (proc-exp bvars (preprocess body)))
      (call-exp (rator rands) (call-exp (preprocess rator) (map preprocess rands)))
      (letrec-exp (p-names b-varss p-bodies letrec-body)
                  (letrec-exp p-names b-varss (map preprocess p-bodies) (preprocess letrec-body)))
      (begin-exp (exp1 exps) (begin-exp (preprocess exp1) (map preprocess exps)))
      (assign-exp (x e) (assign-exp x (preprocess e)))
      (list-exp (exps) (list-exp (map preprocess exps)))
      (null?-exp (exp1) (null?-exp (preprocess exp1)))
      (car-exp (exp1) (car-exp (preprocess exp1)))
      (cdr-exp (exp1) (cdr-exp (preprocess exp1)))
      (cons-exp (arg1 arg2) (cons-exp (preprocess arg1) (preprocess arg2)))
      (new-object-exp (class-name rands) (new-object-exp class-name (map preprocess rands)))
      (self-exp () exp)
      (method-call-exp (obj-exp method-name rands)
                       (method-call-exp (preprocess obj-exp)
                                        (new-method-name method-name (length rands))
                                        (map preprocess rands)))
      (super-call-exp (method-name rands) (super-call-exp (new-method-name method-name (length rands))
                                                          (map preprocess rands))))))
    ...
    (cases
      ...
      (new-object-exp
       ...
       (let ([args (values-of-exps rands env)]
         ...
         (apply-method (find-method class-name
                                    (new-method-name 'initialize (length args)))
                       obj args)
         obj))
