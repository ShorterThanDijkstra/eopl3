  ...
  (lambda (super-m-env new-m-env)
    (define filter
      (lambda (super-m-env)
        (if (null? super-m-env)
            '()
            (let ([m-name (caar super-m-env)])
              (cond [(eqv? m-name 'initialize)
                     (cons (car super-m-env)
                           (filter (cdr super-m-env)))]
                    [(assq m-name new-m-env)
                     (filter (cdr super-m-env))]
                    [else (cons (car super-m-env)
                                (filter (cdr super-m-env)))])))))
    (append new-m-env (filter super-m-env))))
