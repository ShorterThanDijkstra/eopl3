...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (expression ("named-send" identifier
                              expression
                              identifier
                        ...
                        (let ([index (location f-name f-names)])
                          (if index
                              (begin (setref! (list-ref fields index) val)
                                     obj)
                              (eopl:error 'value-of))))))
      (named-send-exp (c-name obj-exp m-name rands)
                      (let ([obj (value-of obj-exp env)]
                            [args (values-of-exps rands env)])
                        (if (instance? obj c-name)
                            (apply-method (find-method c-name m-name) obj  args)
                            (eopl:error 'value-of))))
      (method-call-exp
...
(define apply-method
  (lambda (m self args)
    (cases method
      m
      (a-method (vars body super-name field-names)
                ; (display (= (length field-names)
                ;             (length (object->fields self))))
                ; (newline)
                (value-of body
                          (extend-env
                           vars
...
(define str13
  ...
  let o2 = new c2()
  in named-send c1 o2 foo(7)
  ")
(check-equal? (:e str13) (num-val 10))
