...
(define-datatype
 ...
 (a-method (vars (list-of symbol?))
           ...
           (host-name (maybe symbol?)) ; false for dynamic dispatch
           ))

(define method->maybe-host-name
  (lambda (m)
    (cases method
           m
           (a-method (vars body s-name f-names f-types h-name) h-name))))
...
(define method-decls->method-env
  (lambda (m-decls super-name field-names field-types host-name)
    (map
     (lambda (m-decl)
       (cases
        method-decl
        ...
        (a-method-decl
         ...
         (list method-name
               (a-method vars body super-name field-names field-types #f)))
        (a-static-method-decl
         (result-type method-name vars var-types body)
         (list
          method-name
          (a-method vars body super-name field-names field-types host-name)))))
     m-decls)))
...
(define-datatype environment
                 ...
                 (extend-env-rec** (proc-names (list-of symbol?))
                                   ...
                                   (b-vartypess (list-of (list-of type?)))
...
(define env->list
  (lambda (env)
    (cases environment
           ...
           (extend-env-rec**
            (p-names b-varss b-vartypess p-bodies saved-env saved-tenv)
            (cons (list 'letrec p-names '...) (env->list saved-env)))
           (extend-env-with-self-and-super
            (self super-name saved-env)
            (cons (list 'self self 'super super-name) (env->list saved-env))))))
    ...
    (cases expval
           ...
           (proc-val (p)
                     (cases proc
                            p
                            (procedure
                             (var tys body saved-env saved-tenv)
  ...
  (lambda (pgm)
    ...
    (cases program
           ...
           (a-program (class-decls body)
                      ...
                      (initialize-static-class-env! class-decls)
                      (value-of body (init-env) (init-tenv))))))
...
(define value-of
  (lambda (exp env tenv)
    (cases
     ...
     (diff-exp (exp1 exp2)
               (let ([val1 (expval->num (value-of exp1 env tenv))]
                     [val2 (expval->num (value-of exp2 env tenv))])
                 (num-val (- val1 val2))))
     (sum-exp (exp1 exp2)
              (let ([val1 (expval->num (value-of exp1 env tenv))]
                    [val2 (expval->num (value-of exp2 env tenv))])
     ...
     (if-exp (exp0 exp1 exp2)
             (if (expval->bool (value-of exp0 env tenv))
                 (value-of exp1 env tenv)
                 (value-of exp2 env tenv)))
     (let-exp
      (vars exps body)
      (let ([new-env
             (extend-env vars (map newref (values-of-exps exps env tenv)) env)]
            [new-tenv (extend-tenv
                       vars
                       (map (lambda (exp1) (type-of exp1 tenv)) exps)
                       tenv)])
        (value-of body new-env new-tenv)))
     (proc-exp (bvars types body)
               (proc-val (procedure bvars types body env tenv)))
     (call-exp (rator rands)
               (let ([proc (expval->proc (value-of rator env tenv))]
                     [args (values-of-exps rands env tenv)])
                 (apply-procedure proc args)))
     (letrec-exp
      (result-types p-names b-varss b-vartypess p-bodies letrec-body)
      (let ([new-tenv (extend-tenv p-names
                                   (map (lambda (result-type b-vartypes)
                                          (proc-type b-vartypes result-type)))
                                   tenv)])
        (value-of
         letrec-body
         (extend-env-rec** p-names b-varss b-vartypess p-bodies env new-tenv)
         new-tenv)))
     (begin-exp
      (exp1 exps)
      (letrec ([value-of-begins
                (lambda (e1 es)
                  (let ([v1 (value-of e1 env tenv)])
                    (if (null? es) v1 (value-of-begins (car es) (cdr es)))))])
        (value-of-begins exp1 exps)))
     (assign-exp (x e)
                 (begin
                   (setref! (apply-env env x) (value-of e env tenv))
                   (num-val 27)))
     (null?-exp (exp1)
                (let ([lst (value-of exp1 env tenv)])
                  (bool-val (null? (expval->list lst)))))
     (car-exp (exp1)
              (let ([lst (expval->list (value-of exp1 env tenv))])
                (if (null? lst) (eopl:error 'value-of) (car lst))))
     (cdr-exp (exp1)
              (let ([lst (expval->list (value-of exp1 env tenv))])
                (if (null? lst) (eopl:error 'value-of) (list-val (cdr lst)))))
     (cons-exp (exp1 exp2)
               (let ([val (value-of exp1 env tenv)]
                     [lst (expval->list (value-of exp2 env tenv))])
                 (list-val (cons val lst))))
     ;; args need to be non-empty for type checker
     (list-exp (exp exps)
               (list-val (cons (value-of exp env tenv)
                               (values-of-exps exps env tenv))))
     (new-object-exp
      (class-name rands)
      (let ([args (values-of-exps rands env tenv)]
            [obj (new-object class-name)])
        (apply-method (find-method class-name 'initialize)
                      obj
                      args
                      (map (lambda (exp1) (type-of exp1 tenv)) rands)
                      tenv)
        obj))
     ...
     (method-call-exp
      (obj-exp method-name rands)
      (let ([args (values-of-exps rands env tenv)]
            [obj (value-of obj-exp env tenv)])
        (let ([m (find-method (object->class-name obj) method-name)])
          (let ([host-name-maybe (method->maybe-host-name m)])
            (if host-name-maybe
                (let ([c-name (type->class-name (type-of obj-exp tenv))])
                  (let ([m (find-method c-name method-name)])
                    (apply-method
                     m
                     obj
                     args
     ...
     (super-call-exp
      (method-name rands)
      (let ([args (values-of-exps rands env tenv)] [obj (apply-env env '%self)])
...
(define apply-procedure
  (lambda (proc1 args)
    (cases proc
           proc1
           (procedure
            (vars tys body saved-env saved-tenv)
            (let ([new-env (extend-env vars (map newref args) saved-env)])
              (value-of body new-env (extend-tenv vars tys saved-tenv)))))))

;; apply-method : Method * Obj * Listof(ExpVal) -> ExpVal
(define apply-method
  (lambda (m self args args-types tenv) ;;;how to extend tenv?
    (cases
     method
     m
     (a-method
      (vars body super-name f-names f-types host-name)
      (let ([new-tenv (extend-tenv
                       vars
                       args-types
                       (extend-tenv-with-self-and-super
                        (class-type (object->class-name self))
                        super-name
        ...
        (value-of
         ...
         (extend-env vars
                     ...
                     (extend-env-with-self-and-super
                      self
                      super-name
                      (extend-env f-names (object->fields self) (empty-env))))
         new-tenv))))))
;; exercise: add instrumentation to apply-method, like that for
;; apply-procedure.

(define values-of-exps
  (lambda (exps env tenv) (map (lambda (exp) (value-of exp env tenv)) exps)))

;; is-subclass? : ClassName * ClassName -> Bool
;; c-name1 is subclass c-name2?
(define is-subclass?
...
(define str13
  "let l1 = list(37, 73, 13)
   in let l2 = cdr(cdr(cdr(l1)))
   in list(car(l1), null?(l2))
  ")
(check-equal? (:e str13) (list-val (list (num-val 37) (bool-val #t))))
;  (:t str13) ; should fail
...
(define str15
  ...
  class c2 extends c1
   ...
   in cast o1 c1
  ")
(check-equal? (:t str15) (class-type 'c1))
(check-equal? (:e str15) (an-object 'c2 '()))
...
(define str16
  ...
  class c2 extends c1
   ...
   let o2 = new c2()
   in send cast o2 c1 m()
  ")
(check-equal? (:t str16) (int-type))
(check-equal? (:e str16) (num-val 73))
...
(define str18
  ...
  o = new c2()
  in list((f o), (g o))")
(check-equal? (:e str18) (list-val (list (num-val 12) (num-val 21))))

;;; I kept type infomation at runtime. This is a dump idea.
;;; I should inline static methods
